...
def split_address(addr: str):
    if USE_POSTAL:
        res = {k: "" for k in ("num", "voie", "cp", "ville")}
        res["pays"] = "FR"
        for v, l in parse_address(addr or ""):
            if l == "house_number":
                res["num"] = v
            elif l in {"road", "footway", "path"}:
                res["voie"] = v
            elif l == "postcode":
                res["cp"] = v
            elif l in {"city", "town", "village", "suburb"}:
                res["ville"] = v
            elif l == "country":
                res["pays"] = v
        return res
...
def build_tables(df: pd.DataFrame):
    ...
    pf1 = pd.DataFrame(columns=[
        "uid", "name", "locName", "CXmIAssignedConfiguration", "pcCompoundProfile", "ViewMasterCatalog",
    ])
    pf2 = pd.DataFrame(columns=[
        "B2B Unit", "ADRESSE / Numéro de rue", "ADRESSE / rue", "ADRESSEE Code postal", "ADRESSE / Ville", "ADRESSE / Pays/Région", "INFORMATIONS D'ADRESSE SUPPLÉMENTAIRES / Téléphone 1",
    ])
